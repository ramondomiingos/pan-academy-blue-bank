...
ALTER TABLE `addresses` ADD FOREIGN KEY (`client_id`) REFERENCES `clients` (`client_id`);
...
ALTER TABLE `transactions` ADD FOREIGN KEY (`type_id`) REFERENCES `transaction_type` (`type_id`);

ALTER TABLE `users` ADD FOREIGN KEY (`client_id`) REFERENCES `clients` (`client_id`);

INSERT IGNORE INTO `users` (`user_id`, `username`, `password`, `role`) values (1, 'admin', '$2a$10$hlUaPFEckFJCWKH9rLnuMuhldK3PVisBFJHmZ7H88hKCOpYlm7UIa', 'A');CREATE TABLE IF NOT EXISTS `addresses` (
  `address_id` int PRIMARY KEY AUTO_INCREMENT,
  `address` varchar(255) NOT NULL,
  `address_number` varchar(255),
  `city` varchar(255) NOT NULL,
